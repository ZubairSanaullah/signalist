...
export async function getQuote(symbol: string): Promise<FinnhubQuote | null> {
  try {
    ...
    const data = await fetchJSON<FinnhubQuote>(url, 60); // Cache for 1 minute
    return data;
  } catch (err) {
...
export async function getQuotes(symbols: string[]): Promise<Record<string, FinnhubQuote | null>> {
